...
**Never touch `u.*` inside a `tea.Cmd` closure.** Bubble Tea runs command closures on their own goroutines, so any read or write of `Ui` state from inside one races the main loop. Mutate state in the function body *before* returning the closure, capture what the closure needs as locals, and carry results back as a message that `Update` applies. `startExec`, `startChat`, `execCommand`, and `editSettings` all follow this shape.

Confirmed commands run through `tea.ExecProcess` (via `run.PrepareShellCommand`), which hands the child the real terminal so `sudo`, `vim`, and `top` work. The trade-off is that their output is no longer captured, so only the exit status reaches the model in REPL follow-ups. Restoring output capture would require a PTY.
...
Note: `ChatCompletion` is a single non-streaming `GenerateContent` call that returns via `parallelFallbackMsg`. An earlier streaming scaffold (`engine.channel`, `EngineChatStreamOutput`, `awaitChatStream`) was removed because nothing ever fed the channel, so each chat message parked a goroutine forever. Adding real streaming means introducing a channel *and* a `tea.Cmd` that re-arms itself per chunk.
